...
const Tuple = function () {
  ...
  if (assertBool($gt(fields["length"], 4))) {
    const err = ["You tried to store more than 4 fields in a tuple.", "This probably isn't a good idea.", "Tuples are an effective way to store immutible and fixed length data.", "However, they only make sense for small amounts of data.", "A point could be a tuple: (x, y)", "So could a color: (r, g, b)", "Or even: (r, g, b, a)", "However, once you get past small amounts of data, tuples become confusing.", "If you want an immutable data structure, look into the stone function and objects."]["join"]("\n");
    throw new Error(err);
  }
  let res = {};
  ...
  res["concat"] = function (tuple) {
    ...
    if (assertBool($eq(tuple["length"], 1))) {
      ...
      if (assertBool($eq(tuple["length"], 2))) {
        return Tuple($plus(res["_1"], tuple["_1"]), $plus(res["_2"], tuple["_2"]));
      } else {
        if (assertBool($eq(tuple["length"], 3))) {
          return Tuple($plus(res["_1"], tuple["_1"]), $plus(res["_2"], tuple["_2"]), $plus(res["_3"], tuple["_3"]));
        } else {
          return Tuple($plus(res["_1"], tuple["_1"]), $plus(res["_2"], tuple["_2"]), $plus(res["_3"], tuple["_3"]), $plus(res["_4"], tuple["_4"]));
        }
      }
    }
  };
  res["toString"] = function () {
    return "Tuple("["concat"](Object["entries"](res)["filter"](function ([k, v]) {
