...
var Actor = function (onMessage, state = {}) {
  if (!($eq(typeOf(onMessage), "function"))) { throw new Error("Enter failed") }
  if (!($eq(onMessage["length"], 3))) { throw new Error("Enter failed") }
  let mailbox = [];
  let listening = false;
  ...
  var listen = function (whenDone = function () {
    ...
    if (!($eq(typeOf(whenDone), "function") && $eq(whenDone["length"], 1))) { throw new Error("Enter failed") }
...
var sendAll = function (messages, actor) {
  if (!($eq(typeOf(messages), "array"))) { throw new Error("Enter failed") }
  if (!($eq(typeOf(actor), "Actor"))) { throw new Error("Enter failed") }
...
var $or$lt$lt = function (actor, listener = function (_) {
  ...
  if (!($eq(typeOf(actor), "Actor"))) { throw new Error("Enter failed") }
  if (!($eq(typeOf(listener), "function") && $eq(listener["length"], 1))) { throw new Error("Enter failed") }
