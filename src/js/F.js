...
const map = curry(function (f, arr) {
  if (!($eq(typeOf(f), "function"))) { throw new Error("Enter failed") }
  if (!($eq(typeOf(arr), "array"))) { throw new Error("Enter failed") }
  return function(){
  const res = [];
    for (const a of arr) {
...
const invoke = curry(function (args, f) {
  if (!($eq(typeOf(args), "array"))) { throw new Error("Enter failed") }
  if (!($eq(typeOf(f), "function"))) { throw new Error("Enter failed") }
  return f(...args);
});
...
const predArrayTest = curry(function (method, preds, arr) {
  if (!($eq(typeOf(method), "string"))) { throw new Error("Enter failed") }
  if (!($eq(typeGeneric(preds), "array<function>"))) { throw new Error("Enter failed") }
  if (!($eq(typeOf(arr), "array"))) { throw new Error("Enter failed") }
...
const bothTwo = curry(function (preds, arr) {
  if (!($eq(typeGeneric(preds), "array<function>"))) { throw new Error("Enter failed") }
  if (!($eq(typeOf(arr), "array"))) { throw new Error("Enter failed") }
  if (!($eq(preds["length"], 2))) { throw new Error("Enter failed") }
...
const contains = curry(function (v, col) {
  return matcher([
    [matcher.obj(matcher.prop("includes", matcher.type("function", ""))), function () {
      return col["includes"](v);
    }],
    [matcher.obj(matcher.prop("contains", matcher.type("function", ""))), function () {
      return col["contains"](v);
    }],
    [matcher.wildcard("_"), function (_) {
      return _raise("Cannot check for element presence with contains or includes method.");
    }]])(col);
});
...
const methodInvoke = curry(function (methodName, arg, context) {
  if (assertBool(and(context[methodName], $eq(typeOf(context[methodName]), "function")))) {
    return context[methodName](arg);
  }
  _raise($plus$plus($plus$plus($plus$plus($plus$plus("The object ", context), " has no method called "), methodName), "."));
});
...
const fromEntries = function (entries) {
  if (!($eq(typeOf(entries), "array"))) { throw new Error("Enter failed") }
  const res = {};
...
const ifElse = curry(function (pred, _if, _else) {
  if (!($eq(typeOf(pred), "function"))) { throw new Error("Enter failed") }
  if (!($eq(typeOf(_if), "function"))) { throw new Error("Enter failed") }
  if (!($eq(typeOf(_else), "function"))) { throw new Error("Enter failed") }
  if (assertBool(pred())) {
    _if();
  } else {
    _else();
...
const pipe = function (fns) {
  if (!($eq(typeGeneric(fns), "array<function>"))) { throw new Error("Enter failed") }
  return function (val) {
...
const range = curry(function (from, to) {
  return Array($plus($minus(to, from), 1))["fill"]()["map"](function (_, index) {
    return $plus(index, from);
  });
});
const sort = curry(function (c, arr) {
  if (!($eq(typeOf(c), "function"))) { throw new Error("Enter failed") }
  if (!($eq(typeOf(arr), "array"))) { throw new Error("Enter failed") }
  return [...arr]["sort"](c);
});
const sortBy = sort;
...
const take = curry(function (amount, col) {
  if (!($eq(typeOf(amount), "number"))) { throw new Error("Enter failed") }
...
const test = curry(function (regex, str) {
  if (!($eq(typeOf(str), "string"))) { throw new Error("Enter failed") }
...
const without = curry(function (exclude, arr) {
  if (!($eq(typeOf(exclude), "array"))) { throw new Error("Enter failed") }
  if (!($eq(typeOf(arr), "array"))) { throw new Error("Enter failed") }
  return function(){
  const res = [];
    for (const a of arr) {
...
const takeWhile = curry(function (taker, arr) {
  if (!($eq(typeOf(taker), "function"))) { throw new Error("Enter failed") }
  if (!($eq(typeOf(arr), "array"))) { throw new Error("Enter failed") }
  let res = [];
...
const dropWhile = curry(function (dropper, arr) {
  if (!($eq(typeOf(dropper), "function"))) { throw new Error("Enter failed") }
  if (!($eq(typeOf(arr), "array"))) { throw new Error("Enter failed") }
  let res = [];
...
const zip = curry(function (as, bs) {
  if (!($eq(typeOf(as), "array"))) { throw new Error("Enter failed") }
  if (!($eq(typeOf(bs), "array"))) { throw new Error("Enter failed") }
  const entrs = entries(as);
  const trimEnd = dropWhile(function (arr$exclam) {
    return arr$exclam["includes"](undefined);
  });
  return $or$gt($or$gt($or$gt(id(function(){
  const res = [];
...
const zipWith = curry(function (f, as, bs) {
  ...
  return id($or$gt($or$gt($or$gt(id(function(){
  const res = [];
    for (const [i, a] of entrs) {
        const b = bs[i];
        if (true) res.push([a, b])
  ...
  return res;
}()), reverse), trimEnd), reverse))["map"](function ([a, b]) {
    return f(a, b);
  });
});
