...
const mkView = function (txts) {
  ...
  return {
    ["sentences"]: function () {
      ...
      sc = stone(sentences["map"](function (obj) {
        return obj["trim"]();
      }));
      return sc;
    },
    ["thingsAbout"]: function (thing) {
      if (!($eq(typeOf(thing), "string"))) { throw new Error("Enter failed") }
      thing = RegExp(thing);
...
const scrapy = function (url, ch = chan()) {
  const main = async function () {
    ...
    const findText = function () {
      if (assertBool(and(and($eq(this["nodeType"], 3), not($eq(this["parent"]["type"], "script"))), not(JS["=="](this["data"], 0))))) {
        txts["push"](this["data"]);
      } else {
